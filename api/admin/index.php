    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="glass p-6 rounded-3xl lg:col-span-2">
            <div class="grid grid-cols-3 gap-3 mb-6">
                <?php
                // Hitung total spesifik untuk halaman kas
            ...
            <form action="" method="POST" enctype="multipart/form-data" class="glass p-8 rounded-3xl max-w-2xl border border-white/5 shadow-2xl mx-auto">
                <div class="space-y-4 text-white">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div><label class="text-[9px] text-yellow-500 font-bold ml-1 block uppercase tracking-widest">Logo Web</label><input type="file" name="logo_web" class="w-full bg-white/5 p-2 rounded-xl text-[10px]"></div>
                        <div><label class="text-[9px] text-yellow-500 font-bold ml-1 block uppercase tracking-widest">WhatsApp</label><input type="text" name="wa_admin" value="<?php echo $setting['wa_admin']; ?>" class="w-full bg-white/5 p-3 rounded-xl outline-none border border-white/10 text-sm"></div>
                    </div>
                    <button type="submit" name="update_settings" class="w-full bg-yellow-500 text-black font-black py-4 rounded-xl uppercase text-[11px] tracking-widest mt-4">Save Global Changes</button>
                </div>
            </form>
        </div>
    ...
    <script>
        function showSection(id) {
            ...
            document.querySelectorAll('.nav-link').forEach(l => {
                l.classList.remove('bg-yellow-500', 'text-black', 'shadow-sm');
            ...
            if(target) {
                ...
                if(btn) { 
                    btn.classList.replace('text-gray-400', 'bg-yellow-500'); 
                    btn.classList.add('text-black', 'shadow-sm'); 
                }
                const titles = {'dashboard': 'Dashboard', 'schedules': 'Match Schedules', 'venues': 'Venues Hub', 'kas': 'Accounting Log', 'gallery': 'Gallery Gallery', 'sponsors': 'Sponsor Hub', 'settings': 'Web Settings'};
                document.querySelector('#page-title h2').innerText = titles[id];
                
                // SIMPAN POSISI MENU TERAKHIR
        ...
        function updateClock() {
            const now = new Date();
            // Menambahkan format Hari pada jam realtime
            const days = ['Minggu', 'Senin', 'Selasa', 'Rabu', 'Kamis', 'Jumat', 'Sabtu'];
            const dayName = days[now.getDay()];
            const date = now.toLocaleDateString('id-ID', { day: '2-digit', month: 'long', year: 'numeric' });
            const time = now.toLocaleTimeString('id-ID', { hour12: false });
            
            document.getElementById('realtime-clock').innerText = `${dayName}, ${date} • ${time} WIB`;
        }

        function updatePriceInput() {
            const kategori = document.getElementById('kategori_event').value;
            const flat = document.getElementById('input_flat');
            const pemain = document.getElementById('input_pemain');
            const kiper = document.getElementById('input_kiper');
            if (kategori === 'Futsal') {
                flat.classList.remove('hidden'); flat.required = true;
                pemain.classList.add('hidden'); pemain.required = false;
                kiper.classList.add('hidden'); kiper.required = false;
            } else {
                flat.classList.add('hidden'); flat.required = false;
                pemain.classList.remove('hidden'); pemain.required = true;
                kiper.classList.remove('hidden'); kiper.required = true;
            }
        }

        window.onload = () => {
            // LOAD MENU TERAKHIR
            const lastMenu = sessionStorage.getItem('lastMenu') || 'dashboard';
