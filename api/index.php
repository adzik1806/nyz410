...
<head>
    ...
    100% { transform: translateX(-50%); } /* Berubah ke -50% untuk seamless looping */
}
...
.animate-scroll-fast:hover {
    animation-play-state: paused; /* Opsional: berhenti meluncur saat di-hover */
}

.sponsor-box {
    flex-shrink: 0; /* Mencegah kotak gepeng/hilang */
    background-color: white;
    border-radius: 18px;
    display: flex;
    align-items: center;
...
@media (min-width: 768px) {
    .sponsor-box { width: 200px; height: 100px; padding: 20px; border-radius: 24px; }
}
    </style>
</head>
<body class="overflow-x-hidden">
    ...
    <section class="h-screen flex items-center justify-center relative px-6 overflow-hidden">
        <div class="absolute inset-0 z-0">
            ...
            <img src="https://images.unsplash.com/photo-1543351611-58f69d7c1781?q=80&w=2070" class="w-full h-full object-cover opacity-30" alt="Hero Background">
        </div>
    ...
    <div class="marquee-container relative w-full">
        <div class="animate-scroll-fast flex items-center">
            ...
            // Jika database tidak kosong, kita tampilkan
            if(!empty($sponsors)):
                // Trik Seamless Marquee: Kita looping array yang sama 2x agar menyambung terus
                $double_sponsors = array_merge($sponsors, $sponsors);
                foreach($double_sponsors as $s): 
                    // Pastikan path gambar benar. Jika di DB hanya nama file (misal: nike.png), ganti menjadi: assets/sponsors/<?php echo $s['logo_icon']; ? >
                    $logo_path = $s['logo_icon']; 
            ?>
                    <div class="sponsor-item flex-shrink-0 flex flex-col items-center mx-4 md:mx-8">
                        <div class="sponsor-box bg-white shadow-lg shadow-black/20">
                            <img src="<?php echo $logo_path; ?>" 
                                 class="w-full h-full object-contain max-h-[50px] md:max-h-[70px]" 
                                 alt="<?php echo $s['nama_sponsor']; ?>"
                                 onerror="this.style.display='none'; this.nextElementSibling.style.display='block';">
                            
                            <span class="hidden text-black font-bold text-xs uppercase tracking-wider"><?php echo $s['nama_sponsor']; ?></span>
                        </div>
                        <span class="mt-3 font-black uppercase tracking-[0.3em] text-[9px] md:text-[10px] text-yellow-500/60"><?php echo $s['nama_sponsor']; ?></span>
                    </div>
            <?php 
                endforeach; 
            else:
                // Tampilan default/placeholder jika di database belum ada data sponsor sama sekali
                for($i=1; $i<=4; $i++):
            ?>
                    <div class="sponsor-item flex-shrink-0 flex flex-col items-center mx-8">
                        <div class="sponsor-box bg-white/5 border border-white/10 flex items-center justify-center">
                            <i class="fa-solid fa-handshake text-yellow-500/30 text-2xl"></i>
                        </div>
                        <span class="mt-3 font-black uppercase tracking-[0.3em] text-[10px] text-gray-600">Sample Partner</span>
                    </div>
            <?php 
                endfor;
            endif; 
    ...
    <section id="events" class="py-32 px-6">
        <div class="max-w-7xl mx-auto text-center mb-16" data-aos="fade-up">
            ...
            <div class="flex justify-center flex-wrap gap-2 mt-8">
                <button onclick="filterMatch('all', event)" class="tab-btn active bg-yellow-500 text-black px-6 py-2 rounded-xl font-bold text-xs uppercase tracking-widest">All</button>
                <button onclick="filterMatch('Sepakbola', event)" class="tab-btn bg-white/5 text-gray-400 px-6 py-2 rounded-xl font-bold text-xs uppercase tracking-widest">Football</button>
                <button onclick="filterMatch('Futsal', event)" class="tab-btn bg-white/5 text-gray-400 px-6 py-2 rounded-xl font-bold text-xs uppercase tracking-widest">Futsal</button>
                <button onclick="filterMatch('Minisoccer', event)" class="tab-btn bg-white/5 text-gray-400 px-6 py-2 rounded-xl font-bold text-xs uppercase tracking-widest">Minisoccer</button>
            </div>
        </div>
        ...
        <div class="max-w-7xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            if($gal) {
                ...
                <div class="group relative overflow-hidden rounded-3xl h-64 shadow-xl border border-white/5" data-aos="zoom-in">
                    <img src="<?php echo $rg['foto']; ?>" class="w-full h-full object-cover transition-all duration-700 group-hover:scale-110" alt="Gallery Photo">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-20">
                <div class="space-y-6">
                    ...
                    <div class="flex gap-4">
                        <a href="#" class="w-10 h-10 rounded-xl glass flex items-center justify-center hover:bg-yellow-500 hover:text-black transition-all" aria-label="Instagram"><i class="fab fa-instagram"></i></a>
                        <a href="#" class="w-10 h-10 rounded-xl glass flex items-center justify-center hover:bg-yellow-500 hover:text-black transition-all" aria-label="TikTok"><i class="fab fa-tiktok"></i></a>
                        <a href="#" class="w-10 h-10 rounded-xl glass flex items-center justify-center hover:bg-yellow-500 hover:text-black transition-all" aria-label="YouTube"><i class="fab fa-youtube"></i></a>
                    </div>
                </div>
                <div>
    ...
    <script>
        ...
        function filterMatch(category, e) {
            ...
            if(e && e.currentTarget) {
                e.currentTarget.classList.add('bg-yellow-500', 'text-black');
            }
